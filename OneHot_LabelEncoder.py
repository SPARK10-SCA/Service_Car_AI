...
string_value = pd.DataFrame(np.c_[x[:,0],x[:,2],x[:,5]])
string_value.columns = ['MODELTYPE','COMPANY','CARNAME']

string_dataframe = pd.get_dummies(string_value) # 'MODELTYPE','COMPANY','CARNAME'에 원핫인코딩 진행

integer_value = np.c_[x[:,1],x[:,3],x[:,4],x[:,-1]] # MILEAGE, FIRSTDAY, REPAIRDAY, HQ
category_value = x[:,6:8] # PART, SEVERITY
...
clean_data.columns = ['-1','-2','-3','-4','-5','-6']

clean_data = pd.concat([clean_data,string_dataframe], axis=1)
...
print("Test dats\'s Accuracy : {}".format(r2_score(y_test, y_predict)))
